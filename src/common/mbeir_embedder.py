...
def generate_embeds_and_ids_for_dataset_with_gather(model, data_loader, device, use_fp16=True):
    ...
    print(f"【RANK-{rank}】函数开始，device={device}, pid={os.getpid()}")
    embedding_tensors = []
    id_list = []
    ...
    for batch in data_loader:
        rank = dist.get_rank() if dist.is_initialized() else 0##############
        print(f"【RANK-{rank}】即将开始第一个 batch，device={device}")#########
        # Used in combination with pin_memory=True
        for key, value in batch.items():
            if isinstance(value, torch.Tensor):
                batch[key] = value.to(device, non_blocking=True)
        ...
        # Enable autocast to FP16
        rank = dist.get_rank() if dist.is_initialized() else 0
        print(f"【RANK-{rank}】即将进 autocast + model.forward，device={device}")
        for k, v in batch.items():
            if isinstance(v, torch.Tensor):
                print(f"  batch[{k}] tensor {v.device} {v.shape}")
            elif isinstance(v, dict):
                for kk, vv in v.items():
                    if isinstance(vv, torch.Tensor):
                        print(f"  batch[{k}][{kk}] tensor {vv.device} {vv.shape}")
        
        with autocast(enabled=use_fp16):
            embeddings_batched, ids_list_batched = model(batch, encode_mbeir_batch=True)
##
        #print(f"【RANK-{rank}】model.forward 完成，embeddings_batched 设备: {embeddings_batched.device}, shape: {embeddings_batched.shape}")
        embeddings_batched = embeddings_batched.detach().clone()#####################
        embedding_tensors.append(embeddings_batched.half())  # We only save FP16 embeddings to save space.
        id_list.extend(ids_list_batched)
    # print("embedding_tensors",embedding_tensors)
    ...
    ####################
    #print(f"[Rank {dist.get_rank()}] embedding_tensor device: {embedding_tensor.device}, expected: {device}")
    #assert embedding_tensor.device == device, f"Rank {rank} embedding_tensor is on {embedding_tensor.device}, expected {device}"
    if dist.is_initialized():
        # First, share the sizes of tensors across processes
        size_tensor = torch.tensor([embedding_tensor.size(0)], dtype=torch.long, device=device)
        ...
        if dist.get_rank() == 0:
            ...
            id_list_gathered = [list() for _ in range(dist.get_world_size())]
            sizes = [torch.zeros(1, dtype=torch.long, device=device) for _ in range(dist.get_world_size())]
            for i, t in enumerate(gathered_embeddings):
                print(f"[Rank 0] gathered_embeddings[{i}] device: {t.device}")
                #assert t.device == device, f"gathered_embeddings[{i}] is on {t.device}, expected {device}"
                #############
        else:
            gathered_embeddings = None
            id_list_gathered = None
        ...
        #print(f"[Rank {dist.get_rank()}] embedding_tensor dtype: {embedding_tensor.dtype}, shape: {embedding_tensor.shape}")##############
        # Gather embeddings from all processes
        dist.gather(embedding_tensor, gather_list=gathered_embeddings, dst=0)
        # Gather ids from all processes
        dist.gather_object(id_list, object_gather_list=id_list_gathered, dst=0)
        ...
        if dist.get_rank() == 0:
            ...
            id_list = [id for sublist in id_list_gathered for id in sublist]
            # print("id_list,",id_list)
            # print("embedding_list.size(0)",embedding_list.size(0))
            assert len(id_list) == embedding_list.size(0)
            # Check unique ids
            assert len(id_list) == len(set(id_list)), "Hashed IDs should be unique"
            print(f"Embedder Log: Finished processing embeddings and ids on rank 0.")
...
def main(config):
    ...
    print(f"【MAIN-RANK-{rank}】main() 开始，pid={os.getpid()}")
    # Set up seed for reproducibility
    import faulthandler, signal, os
    faulthandler.register(signal.SIGUSR1)   #  kill -USR1 <pid> 可立刻打印栈
    # 异常时自动 dump
    faulthandler.enable(file=open(f'rank_{os.environ.get("LOCAL_RANK", 0)}_fatal.log', 'w'))
    ########
    seed = config.seed + dist_utils.get_rank()
    set_seed(seed)
    ...
    # Initialize and load model
    # model = build_model_from_config(config)
    print(f"【MAIN-RANK-{rank}】开始 build_model_from_config")
    model = build_model_from_config(config)
    print(f"【MAIN-RANK-{rank}】build_model_from_config 完成")

    model.eval()
    print(f"【MAIN-RANK-{rank}】model.eval() 完成")
    # model.eval()
    ...
    #     if p.device != device:
    #         raise RuntimeError(f"rank {local_rank} param {name} on {p.device} ≠ {device}")

    # # 4. 如果有 buffer 也断言
    # for name, b in model.named_buffers():
    #     if b.device != device:
    #         raise RuntimeError(f"rank {local_rank} buffer {name} on {b.device} ≠ {device}")

    # # 5. 现在才允许 DDP
    # model = DDP(model, device_ids=[local_rank], output_device=local_rank)
    # if config.dist_config.distributed_mode:
    #     # model = DDP(model, device_ids=[config.dist_config.gpu_id])
    #     print(f"【MAIN-RANK-{rank}】即将包装 DDP")
    #     model = DDP(model, device_ids=[config.dist_config.gpu_id])
    #     print(f"【MAIN-RANK-{rank}】DDP 包装完成")
    print(f"Model is set up on GPU {config.dist_config.gpu_id}.")
...
if __name__ == "__main__":
    ...
    # ！！！【关键修复】！！！
    # 将当前进程绑定到正确的 GPU 设备
    # 这是解决 CUDA 非法内存访问错误的核心
    if config.dist_config.distributed_mode:
        torch.cuda.set_device(config.dist_config.gpu_id)
        print(f"Rank {dist_utils.get_rank()} is set to use GPU {config.dist_config.gpu_id}")
